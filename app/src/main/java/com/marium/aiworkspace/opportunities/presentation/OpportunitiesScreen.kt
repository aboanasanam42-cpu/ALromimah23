    ...
    Scaffold(
        topBar = {
            TopAppBar(
                ...
                actions = { IconButton(onClick = { refreshToken++ }) { Icon(Icons.Default.Refresh, "تحديث") } }
            )
        }
    ) { padding ->
                    ...
                    items(opportunities, key = { it.id }) { job ->
                        ElevatedCard(Modifier.fillMaxWidth()) {
                            Column(Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
                                ...
                                Button(onClick = { if (job.url.isNotBlank()) context.startActivity(Intent(Intent.ACTION_VIEW, Uri.parse(job.url))) }, enabled = job.url.isNotBlank()) {
